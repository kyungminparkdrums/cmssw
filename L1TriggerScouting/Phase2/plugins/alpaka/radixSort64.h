#ifndef HeterogeneousCore_AlpakaInterface_interface_radixSort64_h
#define HeterogeneousCore_AlpakaInterface_interface_radixSort64_h
...
namespace cms::alpakatools {
  ...
  ALPAKA_FN_ACC ALPAKA_FN_INLINE void radixSortImpl(
    ...
    if constexpr (!requires_single_thread_per_block_v<TAcc>) {
      ...
      static_assert(NS > 0 && NS <= totalSortingPassses);
      ...
      constexpr int initialSortingPass = totalSortingPassses - NS;
  ...
  template <typename TAcc,
            typename T,
            int NS = (8 * sizeof(T) + 5) / 6,  // number of significant bytes to use in sorting
  ...
  template <typename TAcc, typename T, int NS = (8 * sizeof(T) + 5) / 6>
  ...
  template <typename T, int NS = (8 * sizeof(T) + 5) / 6>
